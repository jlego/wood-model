...
class Model {
  ...
  async create(data = {}, addLock = true, hascheck = true) {
    if (!data) throw this.ctx.error('create方法的参数data不能为空');
    ...
    if (err) throw this.ctx.error(err);
    const lock = addLock ? await this.ctx.catchErr(this.redis.lock()) : {data: 1};
    if (lock.data) {
      let result = await this.ctx.catchErr(this.db.create(this.getData()));
      if(addLock) this.redis.unlock(lock.data);
      if(result.err) throw this.ctx.error(result.err);
      return result.data;
    }else{
      throw this.ctx.error(lock.err);
    }
  }

  // 更新数据
  async update(data = {}, addLock = true, hascheck = true, isFindOneAndUpdate) {
    if (!data) throw this.ctx.error('update方法的参数data不能为空');
    if(!Util.isEmpty(data)) this.setData(data);
    if (!this.isNew()) {
      let err = hascheck ? this.fields.validate() : false,
        hasSet = false,
      ...
      if (err) {
        throw this.ctx.error(err);
      } else {
        let lock = addLock ? await this.ctx.catchErr(this.redis.lock()) : {data: 1};
        if (lock.data) {
          ...
          const result = await this.ctx.catchErr(this.db[method](idObj, hasSet ? data : { $set: data }));
          if(addLock) this.redis.unlock(lock.data);
          if (result.data){
            return isFindOneAndUpdate ? result.data : idObj;
          }else{
            throw this.ctx.error(result.err);
          }
        }else{
          throw this.ctx.error(lock.err);
        }
  ...
  async save() {
    ...
    if (Util.isEmpty(data) || !data) throw this.ctx.error('save方法的data为空');
    if (!this.isNew()) {
      const updateOk = await this.ctx.catchErr(this.update());
      if (updateOk.err) throw this.ctx.error(updateOk.err);
      return updateOk.data;
    } else {
      const result = await this.ctx.catchErr(this.create());
      if (result.err) throw this.ctx.error(result.err);
      return result.data;
    }
  }
  ...
  async remove(data) {
    ...
    const lock = await this.ctx.catchErr(this.redis.lock());
    if (lock.err) {
      throw this.ctx.error(lock.err);
    }else{
  ...
  exec(oper = 'find', data) {
    ...
    return this.ctx.error(this.ctx.error_code.error_nodata);
  }

  // 查询单条记录
  async findOne(data, addLock = true) {
    const hasLock = addLock ? await this.ctx.catchErr(this.redis.hasLock()) : {};
    if(hasLock.err){
      throw this.ctx.error(hasLock.err);
    }else{
      if (!hasLock.data) {
        ...
        let result = await this.ctx.catchErr(this.exec('findOne', query.toJSON()));
        if(result.err){
          throw this.ctx.error(result.err);
        }else{
  ...
  async findList(data, cacheKey = '', addLock = true) {
    if (!data) throw this.ctx.error('findList方法参数data不能为空');
    let hasLock = addLock ? await this.ctx.catchErr(this.redis.hasLock()) : {};
    if(hasLock.err){
      throw this.ctx.error(hasLock.err);
    }else{
      if (!hasLock.data) {
        ...
        const countResult = await this.ctx.catchErr(counts);
        const docsResult = await this.ctx.catchErr(lists);
        if (docsResult.err || countResult.err) {
          throw this.ctx.error(docsResult.err || countResult.err);
        }else{
